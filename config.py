...
# must be set to True for ML_models operating on feature extracted data
config["feature_extraction"] = True
config["include_ML_models"] = True
config["include_DL_models"] = False
config["include_your_ml_models"] = True
config["include_your_DL_models"] = False
...
def build_file_name():
    ...
    all_EEG_file = (
        all_EEG_file + "_" + "synchronised_" + config["preprocessing"]
    )
    all_EEG_file = all_EEG_file + (
        "_hilbert.npz" if config["feature_extraction"] else ".npz"
    )
    return all_EEG_file
...
config[
    "all_EEG_file"
] = build_file_name()  # or use your own specified file name
...
def create_folder():
    if config["retrain"]:
        ...
        config["model_dir"] = os.path.abspath(
            os.path.join(config["log_dir"], model_folder_name)
        )
        ...
        config["info_log"] = (
            config["model_dir"] + "/" + "inference_info_" + stamp + ".log"
        )
        config["batches_log"] = (
            config["model_dir"] + "/" + "inference_batches_" + stamp + ".log"
        )
