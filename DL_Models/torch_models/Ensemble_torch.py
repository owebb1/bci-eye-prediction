...
class Ensemble_torch:
    """
    The Ensemble is a model itself, which contains a number of models whose
    prediction is averaged (majority decision in case of a classifier).
    """
    ...
    def __init__(
        ...
        **model_params,
        ...
        model_list: optional, give a list of models that should be contained
        in the Ensemble
        ...
        """
        ...
        self.load_file_pattern = re.compile(
            self.model_name[:3] + ".*_nb_._best_model.pth", re.IGNORECASE
        )
        self.models = []
        ...
        if self.model_name == "CNN":
            ...
            from DL_Models.torch_models.InceptionTime.InceptionTime import (
                Inception,
            )

            self.model = Inception
        elif self.model_name == "PyramidalCNN":
            from DL_Models.torch_models.PyramidalCNN.PyramidalCNN import (
                PyramidalCNN,
            )
            ...
            from DL_Models.torch_models.LSTM.lstm import LSTMModel2
    ...
    def fit(self, trainX, trainY, validX, validY):
        ...
        # (batch_size, samples, channels) to (bs, ch, samples)
        # as torch conv layers want it
        validX = np.transpose(validX, (0, 2, 1))
        # (batch_size, samples, channels) to
        # (bs, ch, samples) as torch conv layers want it
        train_dataloader = create_dataloader(
            trainX, trainY, self.batch_size, self.model_name
        )
        validation_dataloader = create_dataloader(
            validX, validY, self.batch_size, self.model_name
        )
        # Fit the models
        for i in range(self.nb_models):
            ...
            logging.info(
                "Start fitting model number {}/{} ...".format(
                    i + 1, self.nb_models
                )
            )
            model = self.model(
            ...
            logging.info(
                "Finished fitting model number {}/{} ...".format(
                    i + 1, self.nb_models
                )
            )
    ...
    def predict(self, testX):
        ...
        testX = np.concatenate(
            (testX, dummy)
        )  # TO ADD batch_size - testX.shape[0]%batch_size
        test_dataloader = create_dataloader(
            testX, testX, self.batch_size, self.model_name, drop_last=False
        )
        pred = None
    ...
    def load(self, path):
        ...
        for file in os.listdir(path):
            ...
            logging.info(
                f"Loading model nb from file {file} and predict with it"
            )
            model = self.model(
                loss=self.loss,
                model_number=0,
