...
import logging
from DL_Models.torch_models.torch_utils.training import (
    train_loop,
    validation_loop,
)
...
class BaseNet(nn.Module):
    ...
    def __init__(
        ...
        if loss == "bce":
            ...
            self.output_layer = nn.Sequential(
                nn.Linear(
                    in_features=self.get_nb_features_output_layer(),
            ...
            from DL_Models.torch_models.torch_utils.custom_losses import (
                angle_loss,
            )
    ...
    def fit(self, train_dataloader, validation_dataloader):
        ...
        optimizer = torch.optim.Adam(
            list(self.parameters()), lr=config["learning_rate"]
        )
        ...
        for t in range(epochs):
            ...
            if not self.early_stopped:
                train_loss_epoch, train_acc_epoch = train_loop(
                    train_dataloader,
                    self.float(),
                    self.loss,
                    self.loss_fn,
                ...
                val_loss_epoch, val_acc_epoch = validation_loop(
